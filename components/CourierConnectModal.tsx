...
interface Props {
  open: boolean;
  onClose: () => void;
  provider: CourierProviderDef;
  account?: CourierAccount;
  onSaved: () => void;
}

export default function CourierConnectModal({ open, onClose, provider, account, onSaved }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (account) {
        await api.patch(`/courier/accounts/${account._id}`, { environment, credentials: values });
        toastSuccess('Courier account updated');
      } else {
        await api.post('/courier/accounts', { provider: provider.key, environment, credentials: values });
        toastSuccess('Courier connected');
      }
      onSaved();
      onClose();
    } catch (err) {
  ...
  const copyWebhookUrl = () => {
    ...
    navigator.clipboard.writeText(account.webhookUrl);
        ...
        {account && (
          <div>
            ...
            <div className="flex items-center gap-2">
              <input readOnly value={account.webhookUrl} className={`${inputCls} bg-zinc-50 dark:bg-zinc-950`} />
              <button
                type="button"
            ...
            <p className="mt-1 text-xs text-zinc-400">
              Paste this into {provider.label}&apos;s merchant portal to receive automatic status updates.
            </p>
          </div>
        )}
